...
Feature: Creating AppScopes in Bot Builder
  ...
  Scenario: Assiging appscopes
    ...
    * print scopeclientId2
    * def Payload = read('AssigingAppScpopes.json')
    * set Payload.bots[0] = SanityBotStreamId
    * set Payload.scope[0].bot = SanityBotStreamId
    * set Payload.appName = scopeappName2
    * print Payload
    Given path '/users/'+botadminUserID1+'/streams/'+SanityBotStreamId+'/sdk/apps/'+scopeclientId2
    And request Payload
    And header Authorization = 'bearer '+botadminaccesstokenuser1
    And header Content-Type = 'application/json'
