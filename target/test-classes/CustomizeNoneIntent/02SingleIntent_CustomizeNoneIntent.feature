Feature: To validate the Customize None Intent Check for the New Bots
  ...
  Scenario: To Login and Validate the Status
    ...
    * def anlsValue = response[0].advancedMLConfigurations[0]._id
    ...
    Given url 'https://staging-bots.korebots.com/api/1.1/users/'+userId+'/builder/noneIntent/sentences'
    And header Content-Type = 'application/json'
    And header bot-language = 'en'
    And header Authorization = 'bearer '+accessToken
    And header AccountId = accountID
    ...
    * def ID = response._id
    ...
    Given url 'https://staging-bots.korebots.com/api/1.1/users/'+userId+'/builder/noneIntent/sentences/'+ID+''
    And header Content-Type = 'application/json'
    And header bot-language = 'en'
    And header Authorization = 'bearer '+accessToken
    And header AccountId = accountID
