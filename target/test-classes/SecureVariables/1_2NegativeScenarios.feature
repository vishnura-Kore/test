@R10.0
Feature: feature to create the  Variable with wrong payload
  ...
  * set Payload.description = botName
  ...
  Scenario: Create a  Variable without using parameter
  * def JavaClass = Java.type('data.HashMap')
  * def streamID = JavaClass.get('streamID')
  * print streamID
  * def botName = JavaClass.get('botName')
  ...
  * def name = JavaClass.generateRandom('number')
  * print name 
  * def Payload = read('WrongPayload.json')
  ...
  * set Payload.value = variableValue
   ...
   Then status 412
   And print response
   And match $..msg == ["Validation errors/ Invalid arguments"]
   And match $..code == [412]
